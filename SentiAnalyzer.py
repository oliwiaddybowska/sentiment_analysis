...
class SentiAnalyzer:
    ...
    def probWordPositiveAndNegative(self, sentidata, word, idxWord):
        ...
        for i in range(len(occurrence)):
            positive = positive + (1 if occurrence[i] == 1 and reviews[i] == 1 else 0) # calculate the probability of positive
        ...
        negative = 0
        for i in range(len(occurrence)):
            negative = negative + (1 if occurrence[i] == 1 and reviews[i] == 0 else 0) # calculate the probability of negative
